 ...
 *
 * Parallel version of DungeonMap for the Dungeon Hunter assignment.
 * Represents the dungeon terrain with thread-safe operations.
...
public class DungeonMapParallel {
    ...
    private int[][] manaMap;
    private int[][] visit;
    private volatile int dungeonGridPointsEvaluated; // volatile for thread safety
     ...
     * Thread-safe version with race condition handled as specified in assignment.
     *
     * @param x The x-coordinate in the dungeon grid.
     * @param y The y-coordinate in the dungeon grid.
    ...
    int getManaLevel( int x, int y) {
        ...
        // Race condition acceptable as per assignment requirements
        manaMap[x][y] = fixedPoint;
        dungeonGridPointsEvaluated++; //keep count (race condition here too, but benign)
