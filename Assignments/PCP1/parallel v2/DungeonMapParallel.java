...
public class DungeonMapParallel {
    ...
    public DungeonMapParallel(double xmin, double xmax, 
        ...
        // ThreadLocalRandom for boss placement
        if (seed == 0) {
            // Use thread-safe RNG for better performance
            this.bossX = xmin + (xmax - xmin) * ThreadLocalRandom.current().nextDouble();
            this.bossY = ymin + (ymax - ymin) * ThreadLocalRandom.current().nextDouble();
        } else {
            // Use seeded Random for reproducibility
            Random rand = new Random(seed);
            this.bossX = xmin + (xmax - xmin) * rand.nextDouble();
