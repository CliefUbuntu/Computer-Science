 ...
 *
 * Usage:
 *   java DungeonHunterParallel <gridSize> <numSearchesFactor> <randomSeed> [threads]
 *
...
public class DungeonHunterParallel {
    ...
    static class HuntTask extends RecursiveTask<int[]> {
        ...
        protected int[] compute() {
            if (hi - lo <= THRESHOLD) {
                ...
                int[] leftRes = left.join(); // join left
    ...
    public static void main(String[] args) {
        ...
        int randomSeed = 0;
        int poolThreads = Runtime.getRuntime().availableProcessors(); // default
        ...
        try {
            ...
            // Optional 4th arg: number of ForkJoin worker threads
            if (args.length == 4) {
                poolThreads = Integer.parseInt(args[3]);
                if (poolThreads <= 0) throw new IllegalArgumentException("Number of threads must be positive.");
            }
        ...
        // Important: to ensure we can validate correctness (same starting locations as serial),
        // use a seeded Random to generate starting positions exactly as the serial program does.
        // When randomSeed == 0 we use ThreadLocalRandom for low contention in parallel runs.
        for (int i = 0; i < numSearches; i++) {
            int r, c;
            if (randomSeed > 0) {
                r = rand.nextInt(dungeonRows);
                c = rand.nextInt(dungeonColumns);
            } else {
                r = ThreadLocalRandom.current().nextInt(dungeonRows);
                c = ThreadLocalRandom.current().nextInt(dungeonColumns);
            }
            searches[i] = new HuntParallel(i + 1, r, c, dungeon);
        }

        // Create ForkJoinPool and run a top-level task. We use the Fork/Join
        // mechanism and join() to synchronize (left.join()).
        ForkJoinPool pool = new ForkJoinPool(poolThreads); // custom parallelism if provided

        tick();
        HuntTask root = new HuntTask(searches, 0, searches.length);
        int[] result = pool.invoke(root); // invoke will fork/compute/join internally
        tock();
